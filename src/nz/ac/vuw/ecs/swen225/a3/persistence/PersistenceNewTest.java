...
public class PersistenceNewTest {
	...
	public static void save(Maze maze, String name) {
		...
		GenericTile[][] board = maze.getBoard();
		List<GenericTile> tiles = new ArrayList<>();

		//Convert the 2d array of tiles into a list of the tiles
		for (int i = 0; i < board.length; i++) {
			for (int j = 0; j < board[i].length; j++) {
				GenericTile t = board[i][j];
				tiles.add(t);
			}
		}
     ...
     * @return a JsonArrayBuillder
     */
    private static JsonArrayBuilder convertTiles(List<GenericTile> list) {

		JsonArrayBuilder tilesBuilder = Json.createArrayBuilder();

		for (GenericTile t : list) {
